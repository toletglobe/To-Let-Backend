...
async function markPropertyAsRented() {
  ...
  try {
    // Fetch cached rented property IDs
    const cachedRentedIds = await redis.get("rented_properties");
    let rentedPropertyIds = cachedRentedIds ? JSON.parse(cachedRentedIds) : [];

    // Query only properties that are not cached
    const propertiesToUpdate = await Property.find({
      rentDueDate: { $lte: now },
      availabilityStatus: "Available",
      _id: { $nin: rentedPropertyIds }, // Exclude already updated properties
    });
    ...
    if (propertiesToUpdate.length > 0) {
      ...
      // Cache updated property IDs
      rentedPropertyIds.push(...propertiesToUpdate.map((p) => p._id.toString()));
      await redis.set("rented_properties", JSON.stringify(rentedPropertyIds));
      await redis.expire("rented_properties", 3600); // Expire after 1 hour

      console.log(`Marked ${propertiesToUpdate.length} properties as rented.`);
    } else {
      console.log("No properties found to update.");
    }
